...
广度优先搜索(BFS)路径规划算法实现
基于《现代机器人学》第10章的描述
如果每条边具有相同的成本，Dijkstra算法简化为广度优先搜索
"""
...
class BFSPlanner:
    ...
    def _edge_cost(self, node1, node2, parent_node=None):
        """计算从node1到node2的边成本（BFS中所有边成本相同）"""
        # BFS中所有边的成本都相同（通常为1）
        return 1.0
    ...
    def plan(self, visualize=False, fig=None, ax=None):
        ...
        while self.OPEN:
            ...
            for nbr in self._get_neighbors(current):
                if nbr in self.CLOSED or nbr in self.OPEN:
                    continue
                
                # 计算到达邻居的成本（BFS中所有边成本相同）
    ...
    def _plot_step(self, fig, ax):
        ...
        ax.set_title('广度优先搜索(BFS) 过程可视化', fontsize=14, fontweight='bold')
